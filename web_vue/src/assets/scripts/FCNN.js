...
import { range, pairWise, flatten } from './util';  // 确保 utils 路径正确

export default function FCNN(container) {
    ...
    let w = container.clientWidth;  // 容器宽度
    let h = container.clientHeight;  // 容器高度
    console.log("w : ", w)
    console.log("h : ", w)
    ...
    var architecture = [8, 12, 8];  // 网络架构
    var betweenNodesInLayer = [20, 20, 20];  // 层间距
    var graph = {};
    var layer_offsets = [];
    ...
    // 渲染神经网络
    function redraw({ architecture_ = architecture, showBias_ = showBias, showLabels_ = showLabels, bezierCurves_ = bezierCurves } = {}) {
    ...
    // 更新节点位置
    function redistribute({ betweenNodesInLayer_ = betweenNodesInLayer, betweenLayers_ = betweenLayers, nnDirection_ = nnDirection, bezierCurves_ = bezierCurves } = {}) {
        ...
        let x = (layer) => layer * (betweenLayers + nodeDiameter) + w / 2 - (betweenLayers * layer_offsets.length / 3);
        ...
        let yt = (layer) => layer * (betweenLayers + nodeDiameter) + h / 2 - (betweenLayers * layer_offsets.length / 3);
    ...
    // 样式设置
    function style({ edgeWidthProportional_ = edgeWidthProportional, edgeWidth_ = edgeWidth, edgeOpacityProportional_ = edgeOpacityProportional, edgeOpacity_ = edgeOpacity, negativeEdgeColor_ = negativeEdgeColor, positiveEdgeColor_ = positiveEdgeColor, edgeColorProportional_ = edgeColorProportional, defaultEdgeColor_ = defaultEdgeColor, nodeDiameter_ = nodeDiameter, nodeColor_ = nodeColor, nodeBorderColor_ = nodeBorderColor, showArrowheads_ = showArrowheads, arrowheadStyle_ = arrowheadStyle, bezierCurves_ = bezierCurves } = {}) {
        // Edge Width
